...
// Quota de téléversement — par UTILISATEUR, pas par adresse IP.
// `requireAuth` s'exécute avant ce limiteur, donc l'identité est connue. La
// clé par défaut (req.ip) faisait partager un seul compteur à toute l'agence,
// qui sort par une adresse unique : un dossier de réception de dix photos en
// consommait le tiers, et trois dossiers bloquaient tout le monde pour l'heure.
// Le plafond est dimensionné pour un dossier photo réel, pas pour une pièce.
const uploadLimiter = rateLimit({
  windowMs: 60 * 60 * 1000,
  limit: 200,
  standardHeaders: true,
  legacyHeaders: false,
  keyGenerator: (req) => req.user?.id ?? "anonyme",
