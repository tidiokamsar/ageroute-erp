...
// ici ET dans le bordereau interne, avec des exigences divergentes : le portail
// en imposait quatre, le circuit de validation en exigeait cinq. Une seule
// déclaration fait désormais foi, et elle porte les deux nomenclatures.
import { TYPES_PIECES, TYPES_PIECES_REQUIS, libellePourType } from "../../lib/pieces-obligatoires";

export const REQUIRED_PORTAIL_DECOMPTE_PIECES = TYPES_PIECES_REQUIS;
...
export const portailDecomptePieceSchema = z.object({
  type: z.enum(TYPES_PIECES as unknown as [string, ...string[]]),
  ...
  .superRefine((pieces, context) => {
    ...
    for (const requiredType of REQUIRED_PORTAIL_DECOMPTE_PIECES) {
      if (!seen.has(requiredType)) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          message: `Pièce obligatoire manquante : ${libellePourType(requiredType)}`,
        });
      }
    }
  });
...
const portailDecompteRequestRawSchema = z.object({
  ...
  // Types réellement définis par l'énumération TypeDecompte du schéma Prisma.
  // La liste précédente inventait DEFINITIF et REGULARISATION — qui auraient
  // échoué à l'insertion — et rejetait PROVISOIRE, INTERMEDIAIRE, FINAL,
  // CLOTURE et APRES_AVENANT, pourtant valides et employés en production.
